    ...
    return (
      <section className="admin-panel-card">
        ...
        <AdminTable>
          ...
          <tbody>
            {reviews.length ? reviews.map((review) => <tr key={review.id}><td><input type="checkbox" /></td><td>{"★".repeat(Number(review.rating || 0))}</td><td>{getText(review.comment, language)}</td><td>{review.productName || "-"}</td><td>{review.customerName || "-"}</td><td><Badge tone={review.status === "rejected" ? "danger" : review.status === "approved" ? "active" : "warning"}>{review.status ? localized(review.status.charAt(0).toUpperCase() + review.status.slice(1), review.status === "pending" ? "معلق" : review.status === "approved" ? "مقبول" : "مرفوض", review.status === "pending" ? "ממתין" : review.status === "approved" ? "מאושר" : "נדחה", language) : localized("Pending", "معلق", "ממתין", language)}</Badge></td><td>{formatDate(review.createdAt, language)}</td><td>{formatDate(review.updatedAt, language)}</td><td><div className="row-actions"><button className="text-action" onClick={() => onModerateReview(review.id, "approved", true)} type="button">{localized("Approve", "قبول", "אשר", language)}</button><button className="text-action danger" onClick={() => onDeleteReview?.(review.id)} type="button">{localized("Delete", "حذف", "מחק", language)}</button></div></td></tr>) : <tr><td colSpan="9">{localized("No reviews yet.", "لا توجد تقييمات بعد.", "אין ביקורות עדיין.", language)}</td></tr>}
          </tbody>
        </AdminTable>
      </section>
    );
      ...
      <section className="admin-panel-card">
        ...
        <AdminTable>
          ...
          <tbody>{displayCustomers.length ? displayCustomers.map((customer) => {
            ...
            return <tr key={`${customer.email}-${customer.phone}`}><td>{customer.name}</td><td>{customer.email}</td><td>{customer.phone}</td><td>{isApi ? <select className="admin-select" value={customer.accountType || "retail"} onChange={(e) => handleAccountTypeChange(customer.id, e.target.value)}><option value="retail">{localized("Retail customer", "عميل عادي", "לקוח רגיל", language)}</option><option value="trader">{localized("Trader", "تاجر", "סוחר", language)}</option><option value="wholesale">{localized("Wholesale", "جملة", "סיטונאי", language)}</option></select> : <Badge>{localized("Retail customer", "عميل عادي", "לקוח רגיל", language)}</Badge>}</td><td><Badge>{customer.status || (customer.isActive !== false ? "Active" : "Inactive")}</Badge></td><td>{customer.orders || customer.orderCount || 0}</td><td>{formatDate(customer.createdAt, language)}</td><td>{formatDate(customer.updatedAt, language)}</td></tr>;
